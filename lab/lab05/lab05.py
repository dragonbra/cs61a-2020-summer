...
def riffle(deck):
    ...
    """
    return [deck[(i // 2) if i % 2 == 0 else (i // 2 + len(deck) // 2)] for i in range(0, len(deck))]
...
def berry_finder(t):
    ...
    if label(t) == 'berry':
        ...
        for branch in branches(t):
            res = res or berry_finder(branch)
        return res
...
def add_trees(t1, t2):
    ...
    return tree(root, [add_trees(s1, s2) for s1, s2 in zip(b1, b2)])
